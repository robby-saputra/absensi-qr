                ...
                @forelse($tahunAjaran as $item)
                    <tr>
                        ...
                        <td class="action-buttons">
                            @if(! $item->aktif)
                                <form method="POST" action="/dashboard/admin/tahun-ajaran/{{ $item->id }}/aktif">
                                    @csrf
                                    <button type="submit" class="btn">Set Aktif</button>
                                </form>
                            @endif
                            <a href="/dashboard/admin/tahun-ajaran/edit/{{ $item->id }}" class="btn edit">Edit</a>
                            <a href="/dashboard/admin/tahun-ajaran/delete/{{ $item->id }}" class="btn hapus" data-confirm="Yakin hapus tahun ajaran ini?">Hapus</a>
                        </td>
                    </tr>
                @empty
                    <tr>
