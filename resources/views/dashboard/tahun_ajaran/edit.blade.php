...
<main id="content" class="content">
    ...
    <div class="panel">
        <form method="POST" action="/dashboard/admin/tahun-ajaran/update/{{ $tahun->id }}" class="admin-form">
            ...
            <label>
                Tanggal Mulai
                <input type="date" name="tanggal_mulai" value="{{ old('tanggal_mulai', $tahun->tanggal_mulai) }}" required>
            </label>

            <label>
                Tanggal Selesai
                <input type="date" name="tanggal_selesai" value="{{ old('tanggal_selesai', $tahun->tanggal_selesai) }}" required>
            </label>

            <label class="check-row">
                <input type="checkbox" name="aktif" value="1" {{ old('aktif', $tahun->aktif) ? 'checked' : '' }}>
