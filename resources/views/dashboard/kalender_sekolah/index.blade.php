...
<main id="content" class="content">
        ...
        <div class="calendar-grid">
            ...
            @for($day = 1; $day <= $daysInMonth; $day++)
                @php
                    ...
                    $events = $kalenderBulan->filter(fn($item) => $item->tanggal_mulai <= $date && $item->tanggal_selesai >= $date);
                @endphp
                <div class="calendar-cell {{ $events->where('jenis', 'libur')->isNotEmpty() ? 'libur' : ($events->where('jenis', 'ujian')->isNotEmpty() ? 'ujian' : ($events->where('jenis', 'kegiatan')->isNotEmpty() ? 'kegiatan' : '')) }}">
                    <strong>{{ $day }}</strong>
    ...
    <div class="overview">
        <form method="POST" action="/dashboard/admin/kalender-sekolah/auto-nasional" class="panel admin-form">
            ...
            <label>
                ...
                <select name="tahun_ajaran_id">
                    ...
                    @foreach($tahunAjaran as $ta)
                        <option value="{{ $ta->id }}" {{ ($tahunAjaranId ?? '') == $ta->id ? 'selected' : '' }}>{{ $ta->nama }} - {{ ucfirst($ta->semester) }}</option>
                    @endforeach
                </select>
            </label>
            <label>
        ...
        <form method="POST" action="/dashboard/admin/kalender-sekolah/import" enctype="multipart/form-data" class="panel admin-form">
            @csrf
            <h3>Import Kalender Excel</h3>
            <p>Kolom: tahun_ajaran, semester, tanggal_mulai, tanggal_selesai, judul, jenis, provinsi, keterangan.</p>
    ...
    <div class="panel">
        ...
        <div class="cards">
            <div class="card"><h3>Libur</h3><p>{{ $kalender->where('jenis', 'libur')->count() }}</p></div>
            <div class="card"><h3>Kegiatan</h3><p>{{ $kalender->where('jenis', 'kegiatan')->count() }}</p></div>
            <div class="card"><h3>Ujian</h3><p>{{ $kalender->where('jenis', 'ujian')->count() }}</p></div>
        </div>
        <table>
            <thead>
                <tr>
                ...
                @forelse($kalender as $item)
                    <tr>
                        ...
                        <td class="action-buttons">
                            <a href="/dashboard/admin/kalender-sekolah/edit/{{ $item->id }}" class="btn edit">Edit</a>
                            <a href="/dashboard/admin/kalender-sekolah/delete/{{ $item->id }}" class="btn hapus" data-confirm="Hapus data kalender ini?">Hapus</a>
                        </td>
                    </tr>
                @empty
                    <tr><td colspan="8" class="empty-table">Belum ada data kalender sekolah.</td></tr>
                @endforelse
            </tbody>
        </table>
    </div>
