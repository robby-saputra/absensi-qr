    ...
    <div class="monitor-page">
        ...
        <section class="monitor-panel">
            ...
            @if($tab === 'mapel')
                <div class="table-wrap">
                    <table class="monitor-table">
                        <thead><tr><th>Guru Utama</th><th>Tanggal</th><th>Kelas & Mapel</th><th>JP/Jam</th><th>Status Efektif</th><th>Pengganti</th><th>Guru Aktif</th><th>Aksi</th></tr></thead>
                        <tbody>
                        @forelse($mapelRows as $row)
                            @php
                                $statusClass = match($row->effective_status ?? $row->status_harian) {
                                    'hadir_otomatis' => 'auto',
                                    'izin' => 'info',
                                    'sakit', 'inval' => 'danger',
                                    'digantikan' => 'purple',
                                    'menunggu_verifikasi_ulang' => 'orange',
                                    'belum_konfirmasi' => 'warn',
                                    default => 'ok',
                                };
                                $modalId = 'cancel-mapel-'.($row->attendance_id ?: 'auto-'.$row->id);
                            @endphp
                            <tr>
                                ...
                                <td><span class="status-badge {{ $statusClass }}">{{ $row->status_label }}</span><br><small>{{ $row->is_automatic_cutoff ? 'Otomatis cutoff' : ($row->status_dipilih_at ? \Carbon\Carbon::parse($row->status_dipilih_at)->format('d/m/Y H:i') : '-') }}</small></td>
                                <td>
                                    <div class="chain">
                                ...
                                <td>
                                    ...
                                    @if($row->can_cancel_verification && $row->cancel_action)
                                        @include('dashboard.admin_teacher_verifications.partials.cancel_modal', [
                                            ...
                                            'statusSource' => $row->status_source ?: ($row->is_automatic_cutoff ? 'otomatis_cutoff' : 'manual'),
                                            'isAutomatic' => $row->is_automatic_cutoff,
                        ...
                        @empty
                            <tr><td colspan="8"><div class="empty-state">Tidak ada jadwal mapel pada tanggal ini.</div></td></tr>
                        @endforelse
                        </tbody>
                    </table>
                </div>
            @else
                <div class="table-wrap">
                    <table class="monitor-table">
                        <thead><tr><th>Guru Utama</th><th>Tanggal</th><th>Hari</th><th>Jam Piket</th><th>Status Efektif</th><th>Cadangan/Pengganti</th><th>Petugas Aktif</th><th>Aksi</th></tr></thead>
                        <tbody>
                        @forelse($piketRows as $row)
                            @php
                                ...
                                $modalId = 'cancel-piket-'.($row->attendance_id ?: 'auto-'.$row->id);
                            @endphp
                            <tr>
                                <td><strong>{{ $row->nama_guru }}</strong></td>
                                <td>{{ \Carbon\Carbon::parse($tanggal)->locale('id')->translatedFormat('l, d F Y') }}</td>
                                ...
                                <td>
                                    <div class="chain">
                                        <span>Cadangan: {{ $row->nama_guru_pengganti ?: 'Tidak ada' }}</span>
                                        <span>Pengganti aktif: {{ optional($row->active_replacement)->nama_pengganti ?: optional($row->active_replacement)->nama_pengganti_rantai ?: 'Tidak ada' }}</span>
                                        @forelse($row->replacement_chain as $r)
                                            <span>Riwayat #{{ $r->urutan_penggantian }} {{ $r->nama_pengganti }} - {{ ucfirst(str_replace('_',' ', $r->status_penugasan)) }}</span>
                                        @empty
                                            <span>Belum ada riwayat pengganti</span>
                                        @endforelse
                                    </div>
                                </td>
                                ...
                                <td>
                                    ...
                                    <div id="detail-piket-{{ $row->id }}" class="inline-detail">
                                        Cadangan: {{ $row->nama_guru_pengganti ?: '-' }}<br>
                                        Pengganti aktif: {{ optional($row->active_replacement)->nama_pengganti ?: optional($row->active_replacement)->nama_pengganti_rantai ?: '-' }}<br>
                                        Pengganti lanjutan: {{ $row->continuations->pluck('nama_pengganti')->implode(', ') ?: '-' }}<br>
                                        Riwayat dibatalkan/berhalangan: {{ $row->replacement_history->pluck('nama_pengganti')->implode(', ') ?: '-' }}<br>
                                        Absensi siswa: {{ $row->has_student_attendance ? 'Sudah ada' : 'Belum ada' }}
                                    </div>
                                    @if($row->can_cancel_verification && $row->cancel_action)
                                        @include('dashboard.admin_teacher_verifications.partials.cancel_modal', [
                                            ...
                                            'penggantiAktif' => optional($row->active_replacement)->nama_pengganti ?: optional($row->active_replacement)->nama_pengganti_rantai ?: '-',
                                            'penggantiLanjutan' => $row->continuations->pluck('nama_pengganti')->implode(', ') ?: '-',
                                            'hasStudentAttendance' => $row->has_student_attendance,
                                            'isAfterCutoff' => $isAfterCutoff,
                                        ])
