...
<div id="content" class="content">
    ...
    <form method="GET" class="table-box" style="display:grid;grid-template-columns:repeat(auto-fit,minmax(180px,1fr));gap:12px;align-items:end">
        <label>Tahun Ajaran
        ...
        <a class="btn" target="_blank" href="/dashboard/wali/laporan-bulanan?bulan={{ now()->format('Y-m') }}&tahun_ajaran_id={{ $tahunAjaranId }}">Laporan Bulanan</a>
    </form>
    ...
    <div class="cards">
        <div class="card"><h3>Hadir 30 Hari</h3><p>{{ (int)($analitik->hadir ?? 0) }}</p></div>
        <div class="card"><h3>Telat</h3><p>{{ (int)($analitik->telat ?? 0) }}</p></div>
        <div class="card"><h3>Izin</h3><p>{{ (int)($analitik->izin ?? 0) }}</p></div>
        <div class="card"><h3>Sakit</h3><p>{{ (int)($analitik->sakit ?? 0) }}</p></div>
        <div class="card"><h3>Alfa</h3><p>{{ (int)($analitik->alfa ?? 0) }}</p></div>
    </div>
    ...
    <div class="table-box">
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:18px">
                ...
                @foreach($trenMingguan as $t)
                    <div style="margin:8px 0">
                        ...
                        <div style="height:12px;background:#e5e7eb;border-radius:999px;overflow:hidden">
                            <span style="display:block;height:12px;width:{{ min(100, $t->total * 4) }}%;background:#2563eb"></span>
                        </div>
                    </div>
                @endforeach
            </div>
            <div>
                ...
                <table>
                    <tr><th>Nama</th><th>Total</th><th>Surat</th></tr>
                    ...
                    @empty
                        <tr><td colspan="3">Tidak ada siswa rawan.</td></tr>
                    @endforelse
                </table>
            </div>
        </div>
