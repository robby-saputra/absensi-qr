...
<main id="content" class="content">
    ...
    <form method="POST" action="{{ $mode === 'edit' ? '/dashboard/admin/absensi/update/'.$absensi->id : '/dashboard/admin/absensi/store' }}" class="rekap-filter">
        @csrf
        <select name="tahun_ajaran_id">
            <option value="">Tahun Ajaran Aktif</option>
            @foreach($tahunAjaran as $ta)
                <option value="{{ $ta->id }}" {{ (old('tahun_ajaran_id', $tahunAjaranId) == $ta->id) ? 'selected' : '' }}>
        ...
        <select name="id_siswa" required>
            ...
            @foreach($siswa as $s)
                <option value="{{ $s->id }}" {{ old('id_siswa', $absensi->id_siswa ?? '') == $s->id ? 'selected' : '' }}>
                    {{ $s->nama }} {{ $s->nis ? '- '.$s->nis : '' }}
                </option>
            @endforeach
        </select>
        <input type="date" name="tanggal" value="{{ old('tanggal', $absensi->tanggal ?? now()->toDateString()) }}" required>
        ...
        <select name="status_masuk">
            ...
            @foreach(['hadir','telat','izin','sakit','alfa'] as $status)
                <option value="{{ $status }}" {{ old('status_masuk', $absensi->status_masuk ?? '') === $status ? 'selected' : '' }}>{{ ucfirst($status) }}</option>
            @endforeach
        </select>
        ...
        <select name="status_pulang">
            ...
            @foreach(['pulang','pulang_cepat','izin','sakit','alfa'] as $status)
                <option value="{{ $status }}" {{ old('status_pulang', $absensi->status_pulang ?? '') === $status ? 'selected' : '' }}>{{ ucfirst(str_replace('_', ' ', $status)) }}</option>
            @endforeach
        </select>
