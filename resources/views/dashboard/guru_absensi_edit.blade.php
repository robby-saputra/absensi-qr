...
<main id="content" class="content">
    <div class="attendance-panel">
        ...
        <form method="POST" action="/dashboard/guru/absensi/{{ $siswa->id }}/update" class="filter-box edit-absensi-form">
            @csrf
            ...
            <label>
                Jam Masuk
                <input type="time" name="jam_masuk" value="{{ old('jam_masuk', $absensi ? substr((string) $absensi->jam_masuk, 0, 5) : '') }}">
            </label>
            ...
            <label>
                ...
                <select name="status_masuk">
                    ...
                    @foreach(['hadir','telat','izin','sakit'] as $status)
                        <option value="{{ $status }}" {{ old('status_masuk', $absensi->status_masuk ?? '') == $status ? 'selected' : '' }}>
                            {{ ucfirst($status) }}
                        </option>
                    @endforeach
                </select>
            ...
            <label>
                Jam Pulang
                <input type="time" name="jam_pulang" value="{{ old('jam_pulang', $absensi ? substr((string) $absensi->jam_pulang, 0, 5) : '') }}">
            </label>
            ...
            <label>
                ...
                <select name="status_pulang">
                    ...
                    @foreach(['pulang','izin','sakit'] as $status)
                        <option value="{{ $status }}" {{ old('status_pulang', $absensi->status_pulang ?? '') == $status ? 'selected' : '' }}>
                            {{ ucfirst($status) }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="filter-actions">
                ...
                <a href="/dashboard/guru/absensi/{{ $siswa->id }}/view?tanggal={{ $tanggal }}" class="btn btn-success">View</a>
            </div>
        </form>
    </div>
</main>
