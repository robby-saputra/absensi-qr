...
<div class="container">
    ...
    <form action="/dashboard/admin/guru-piket/update/{{ $guruPiket->id }}" method="POST">
        ...
        <select name="guru_id" required>
            @foreach($guru as $g)
                <option value="{{ $g->id }}" {{ old('guru_id', $guruPiket->guru_id) == $g->id ? 'selected' : '' }}>
                    {{ $g->nama }} ({{ $g->username }})
                </option>
            @endforeach
        </select>
        ...
        <select name="guru_pengganti_id">
            ...
            @foreach($guru as $g)
                <option value="{{ $g->id }}" {{ old('guru_pengganti_id', $guruPiket->guru_pengganti_id) == $g->id ? 'selected' : '' }}>
                    {{ $g->nama }} ({{ $g->username }})
                </option>
            @endforeach
        </select>
        ...
        <select name="guru_pengganti2_id">
            <option value="">Tidak Ada</option>
            @foreach($guru as $g)
                <option value="{{ $g->id }}" {{ old('guru_pengganti2_id', $guruPiket->guru_pengganti2_id) == $g->id ? 'selected' : '' }}>
                    {{ $g->nama }} ({{ $g->username }})
                </option>
            @endforeach
        </select>
        ...
        <select name="hari" required>
            @foreach(['Senin','Selasa','Rabu','Kamis','Jumat','Sabtu'] as $h)
                <option value="{{ $h }}" {{ old('hari', ucfirst($guruPiket->hari)) == $h ? 'selected' : '' }}>
                    {{ $h }}
                </option>
            @endforeach
        </select>
        ...
        <input type="time" name="jam_mulai" value="{{ old('jam_mulai', substr($guruPiket->jam_mulai, 0, 5)) }}" required>

        <label>Jam Selesai</label>
        <input type="time" name="jam_selesai" value="{{ old('jam_selesai', substr($guruPiket->jam_selesai, 0, 5)) }}" required>

        <label>Status</label>
        <select name="status" required>
            @foreach(['Akan Bertugas','Sedang Bertugas','Izin','Sakit','Digantikan','Selesai'] as $status)
                <option value="{{ $status }}" {{ old('status', $guruPiket->status) == $status ? 'selected' : '' }}>
                    {{ $status }}
                </option>
            @endforeach
        </select>
        ...
        <label class="guru-item">
            <input type="checkbox" name="aktif" value="1" {{ old('aktif', $guruPiket->aktif) ? 'checked' : '' }}>
            <div>Aktif</div>
        </label>
