...
<main id="content" class="content">
    <div class="welcome">
        ...
        <div>
            ...
            @if($layout !== 'wali')
                <a href="/dashboard/admin/siswa/detail/{{ $siswa->id }}/pdf" target="_blank" class="btn">PDF Resmi</a>
            @else
                <a href="/dashboard/wali/pdf/absensi?tanggal={{ now()->toDateString() }}" target="_blank" class="btn">PDF Rekap Kelas</a>
                <a href="/dashboard/wali/surat/{{ $siswa->id }}" target="_blank" class="btn">Surat Pembinaan</a>
            @endif
        </div>
    </div>
    ...
    @if($layout === 'wali')
        <div class="panel">
            ...
            <table>
                <tr><th>Tanggal</th><th>Jenis</th><th>Status</th><th>Alasan</th><th>Bukti</th></tr>
                ...
                @empty
                    <tr><td colspan="5" class="empty-table">Belum ada pengajuan izin/sakit.</td></tr>
                @endforelse
            </table>
        </div>
        ...
        <div class="panel">
            ...
            <form method="POST" action="/dashboard/wali/siswa/{{ $siswa->id }}/catatan" class="filter-box">
                @csrf
                <label>Tanggal <input type="date" name="tanggal" value="{{ now()->toDateString() }}" required></label>
                <label>Kategori
                ...
                <label>Catatan <textarea name="catatan" rows="3" required placeholder="Tulis tindak lanjut wali kelas"></textarea></label>
            ...
            <table>
                ...
                @empty
                    <tr><td colspan="3" class="empty-table">Belum ada catatan pembinaan.</td></tr>
                @endforelse
            </table>
        </div>
    @endif
