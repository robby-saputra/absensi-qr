...
<body>
    ...
    <main id="content" class="content">
        ...
        <div class="attendance-panel">
            ...
            <form method="POST" action="/dashboard/guru/absensi-mapel/{{ $jadwal->id }}/{{ $siswa->id }}/update"
                ...
                <label>Tanggal<input type="date" name="tanggal" value="{{ old('tanggal', $tanggal) }}"
                        required></label>
                <label>Jam Absen Mapel<input type="time" name="jam_scan"
                        value="{{ old('jam_scan', $absensiMapel ? substr((string) $absensiMapel->jam_scan, 0, 5) : '') }}"></label>
                <label>
                    Status Absen Mapel
                    <select name="status" required>
                        @foreach (['hadir', 'izin', 'sakit', 'alpa'] as $status)
                            <option value="{{ $status }}"
                                {{ old('status', $absensiMapel->status ?? 'hadir') == $status ? 'selected' : '' }}>
                ...
                <label>Catatan Guru
                    <textarea name="catatan_guru" rows="3"
                        placeholder="Contoh: siswa terlambat masuk kelas, izin UKS, atau keterangan lain">{{ old('catatan_guru', $absensiMapel->catatan_guru ?? '') }}</textarea>
                </label>
                <div class="filter-actions">
                    <button type="submit" class="btn">Simpan Absen Mapel</button>
                    <a href="/dashboard/guru/absensi-mapel/{{ $jadwal->id }}/{{ $siswa->id }}/view?tanggal={{ $tanggal }}"
