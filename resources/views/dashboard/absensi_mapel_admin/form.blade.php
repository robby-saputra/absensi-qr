...
<main id="content" class="content">
    ...
    <form method="POST" action="{{ $mode === 'edit' ? '/dashboard/admin/absensi-mapel/update/'.$absensi->id : '/dashboard/admin/absensi-mapel/store' }}" class="rekap-filter">
        @csrf
        ...
        <select name="jadwal_id" required>
            ...
            @foreach($jadwal as $j)
                <option value="{{ $j->id }}" {{ old('jadwal_id', $absensi->jadwal_id ?? '') == $j->id ? 'selected' : '' }}>
                    {{ $j->nama_kelas }} - {{ $j->nama_mapel }} - {{ ucfirst($j->hari) }} {{ $j->jam_mulai }}-{{ $j->jam_selesai }} - {{ $j->nama_guru }}
                </option>
            @endforeach
        </select>
        ...
        <select name="status" required>
            @foreach(['hadir','telat','izin','sakit','alfa'] as $status)
                <option value="{{ $status }}" {{ old('status', $absensi->status ?? 'hadir') === $status ? 'selected' : '' }}>{{ ucfirst($status) }}</option>
            @endforeach
        </select>
