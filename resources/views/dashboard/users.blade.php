...
<body>
    ...
    <main class="content" style="margin-left:0">
        ...
        <div class="rekap-filter">
            ...
            <span class="status-pill">Masuk: {{ $absensiHariIni->jam_masuk ?? '-' }}
                {{ $absensiHariIni->status_masuk ? '(' . $absensiHariIni->status_masuk . ')' : '' }}</span>
            <span class="status-pill">Pulang: {{ $absensiHariIni->jam_pulang ?? '-' }}
                {{ $absensiHariIni->status_pulang ? '(' . $absensiHariIni->status_pulang . ')' : '' }}</span>
        </div>
        ...
        <table>
            ...
            @empty
                <tr>
                    <td colspan="5" class="empty-row">Belum ada riwayat harian.</td>
                </tr>
            @endforelse
        </table>
        ...
        <table>
            ...
            @empty
                <tr>
                    <td colspan="5" class="empty-row">Belum ada riwayat mapel.</td>
                </tr>
            @endforelse
        </table>
    </main>
