...
// Controller ini menyiapkan halaman rekap admin
// untuk guru piket, absensi mapel, dan jadwal guru.
class RekapAdminController extends Controller
{
    // Menampilkan rekap status guru piket
    // pada tanggal dan filter yang dipilih.
    public function guruPiket(Request $request)
    {
        ...
        // Query ini menggabungkan jadwal piket
        // dengan status kehadiran harian.
        $query = DB::table('guru_pikets as gp')
            ->join('users as g', 'g.id', '=', 'gp.guru_id')
            ->leftJoin(
                'guru_piket_statuses as gps',
                function ($join) use ($tanggal) {
                    $join->on(
                        'gps.guru_piket_id',
                        '=',
                        'gp.id'
            ...
            ->select(
                ...
                'gps.waktu_scan_masuk',
                'gps.waktu_scan_keluar',
    ...
    // Menampilkan rekap absensi mapel berdasarkan
    // tanggal, kelas, mapel, JP, dan status.
    public function absensiMapel(Request $request)
    {
        ...
        // Query ini membandingkan jadwal mapel,
        // absensi harian, dan absensi mapel siswa.
        $query = DB::table(
            ...
            /*
             * JOIN absensi_mapels harus diletakkan
             * sebelum JOIN users as gpel karena
             * gpel menggunakan a.guru_pelaksana_id.
             */
            ->leftJoin(
                'absensi_mapels as a',
                function ($join) use ($tanggal) {
        ...
        // Kondisi ini sebelumnya tertulis dua kali.
        $query
        ...
        if ($tahunAjaranId) {
            $query->where(
                function ($tahun) use ($tahunAjaranId) {
                    $tahun
                        ->where(
                            'j.tahun_ajaran_id',
                            $tahunAjaranId
    ...
    // Menampilkan rekap jadwal guru mata pelajaran.
    public function jadwalGuruMapel(Request $request)
    {
        ...
        return view(
            'dashboard.rekap.jadwal_guru_mapel',
            compact(
                'user',
                'data',
                'guru',
